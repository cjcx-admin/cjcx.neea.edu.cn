...
mt.sequence = function(a, pos, fun) {
    ...
    for (var i = 0; i < a.length; i++) {
        ...
        var td = tr[i].firstChild;
        if (td.className == 'seq') {
            td.innerText = i + 1 + pos;
        } //序号
        tr[i].onmousedown = function(ev) {
            ...
            if (e.name != 'sequence' || event.button > 1) {
                return;
            }
            ...
            for (var j = 0; j < tr.length; j++) {
                tr[j].locy = mt.top(tr[j]) + (j < this.rowIndex ? tr[j].offsetHeight : 0);
            }
            //创建占位符
...
mt.nmark = function(n, g, p) {
    ...
    var pg = mt.f(g / (g + p) * 100, 1) + '%',
        pp = mt.f(p / (g + p) * 100, 1) + '%';
    var dg = $$('_good').childNodes,
        dp = $$('_poor').childNodes;
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        arr[i].onkeyup = function() {
            ...
            if (this.name == 'password') {
                ...
                for (var j = 1; j < 3; j++) {
                    $$('strength' + j).className = i > j ? 'strength2' : '';
                }
            }
        };
        arr[i].onblur = function() {
            var t = this.parentNode,
                v = this.value.trim(),
                min = parseInt(this.getAttribute('min') || '1');
...
//填充
mt.detail = function() {
    var tr = $$('tablecenter').firstChild.childNodes;
    alert($$('tablecenter').firstChild);
...
mt.fill = function(f) {
    ...
    for (var i = 0, j = 100, s = 0; i < f.length; i++) {
        if (/hidden|submit|button/.test(f[i].type)) {
            continue;
        }
        if (f[i].tagName == 'SELECT') {
            if (f[i].options.length <= ++s) {
                continue;
            }
            f[i].selectedIndex = s;
            if (f[i].onchange) {
...
//排序
mt.order = function(a) {
    var v1 = a.substring(1),
        v2 = form1.order.value;
    if (v1 == v2) {
...
mt.toast = function(s, u) {
    ...
    _toast.inter = setTimeout(function() {
        _toast.style.display = 'none';
        if (u) {
            location.replace(u)
        }
